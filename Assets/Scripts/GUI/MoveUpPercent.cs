...
public class MoveUpPercent : MonoBehaviour {
	...
	void Start () {
		...
		if(!invert)
			rt.position = new Vector3 (rt.position.x, rt.position.y - (Screen.height*offsetY), rt.position.z);
		StartCoroutine ("anim");
	}
	...
	IEnumerator anim(){
		...
		float from = initialY - (Screen.height*offsetY);
		float to = initialY;
		if (invert) {
			float tmp = from;
			from = to;
